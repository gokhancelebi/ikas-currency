...
class RateService
{
    ...
    {
        $currencyUrl = (string) config('shopify.altinkaynak.currency_url');
        $goldUrl = (string) config('shopify.altinkaynak.gold_url');
        ...
        file_put_contents(
            config('shopify.rates.currency_xml'),
            $this->buildKurlarXml($currencyItems)
        );

        file_put_contents(
            config('shopify.rates.gold_xml'),
            $this->buildKurlarXml($goldItems)
        );
    }

    /**
     * JSON kayıtlarını mevcut UI/Sync XML formatına çevirir.
     *
    ...
    /** Altınkaynak TR sayı formatı: 47,086 veya 6.071,34 */
    ...
    /**
     * Ürün listesi: kur dosyaları var mı, geçerli mi, taze mi?
     *
     * @return array{
     *     ready: bool,
     *     message: string,
    ...
    {
        $maxAgeSeconds ??= (int) config('shopify.rates.max_age_seconds', 3600);
        $currencyPath = (string) config('shopify.rates.currency_xml');
        $goldPath = (string) config('shopify.rates.gold_xml');
    ...
    /** XML içindeki zaman ile dosya mtime — hangisi yeniyse onu kullan */
    ...
    public function getRates(): array
    {
        ...
        $currencyPath = config('shopify.rates.currency_xml');
        ...
        $goldPath = config('shopify.rates.gold_xml');
